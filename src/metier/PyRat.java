...
public class PyRat {
    private List<Point> fromageList;
    private Map<Point, List<Point>> laby;
    private Set<Point> fromageSet;
    private Map<Point, Set<Point>> passageOrdreConstant;
    ...
    public void preprocessing(Map<Point, List<Point>> laby, int labyWidth, int labyHeight, Point position, List<Point> fromages) {
        this.fromageSet = new HashSet<>(fromages);
        this.fromageList = fromages;
        this.laby = laby;
        this.passageOrdreConstant = new HashMap<>();
        for (Map.Entry<Point, List<Point>> items : laby.entrySet()) {
            passageOrdreConstant.put(items.getKey(), new HashSet<>(items.getValue()));
        }
    }
    ...
    private boolean fromageIci(Point pos) {
        ...
        return fromageList.contains(pos);
    }
    ...
    private boolean fromageIci_EnOrdreConstant(Point pos) {
        ...
        return fromageSet.contains(pos);
    }
    ...
    private boolean passagePossible_EnOrdreConstant(Point de, Point a) {
        return passageOrdreConstant.get(de).contains(a);
    }
    ...
    private List<Point> pointsInatteignables(Point pos) {
        Set<Point>  labyPoints= new HashSet<>(laby.keySet());
        labyPoints.removeAll(this.backTrack(pos, new HashSet<>()));
        return new ArrayList<>(labyPoints);

    }


    private Set<Point> backTrack(Point pos, Set<Point> atteignable) {
        atteignable.add(pos);
        for (Point point : passageOrdreConstant.get(pos)) {
            if(!atteignable.contains(point)){
                backTrack(point, atteignable);
            }
        }
        return atteignable;
    }
